...
static std::vector<std::string> to_strings(size_t n, const char* strs[]) {
  std::vector<std::string> strs_v;
  strs_v.reserve(n);
  for (unsigned i = 0; i < n; ++i) {
    strs_v.push_back(std::string(strs[i]));
  }
  return strs_v;
}
...
int halo_CompileTFPbGraph(const char* pb_buf, size_t pb_buf_size,
  ...
  const halo::CXXCodeGenOpts& opts = *(halo::CXXCodeGenOpts*)(cg_opts);
  return halo::CompileTFGraph(pb_buf, pb_buf_size,
                              to_strings(num_input_shapes, input_shapes), opts,
