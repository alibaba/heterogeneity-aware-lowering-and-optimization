...
void GenericCXXCodeGen::RunOnInstruction(HgEngineInst* inst) {
  ...
  EmitODLACall(rets, "odla_HgaiEngine", inputs, serialized_engine, input_layout,
               output_layout, inst->GetInBindingList(), in_binding_list_length,
               inst->GetOutBindingList(), out_binding_list_length,
               inst->GetOutTypeList());
}
