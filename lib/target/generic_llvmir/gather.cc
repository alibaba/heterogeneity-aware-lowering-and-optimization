...
void GenericLLVMIRCodeGen::RunOnInstruction(GatherInst* inst) {
  llvm::IRBuilder<>* ir_builder = current_llvm_builder_;
  const Def& params = inst->GetOperand(0);
  const Def& indices = inst->GetOperand(1);
  llvm::Value* op0 = ir_mapping_[params];
  llvm::Value* op1 = ir_mapping_[indices];
  ...
  llvm::FunctionType* ftype = llvm::FunctionType::get(
      ...
      {data_ptr_type, data_ptr_type, int32_ptr_ty, int64_ty, int64_ty}, false);
  ...
  HLCHECK(inst->GetAxis() == 0 && "unsupported axis value for gather.");
  auto size_on_axis = params.GetType().GetNumOfElementsInDim(0);
  auto size_after_axis =
      params.GetType().GetTotalNumOfElements() / size_on_axis;
  llvm::Value* col = ir_builder->getInt64(size_after_axis);
  ...
  CreateCall(&callee, {ret_buf_ptr, input0, input1, col, noe_indicies});
