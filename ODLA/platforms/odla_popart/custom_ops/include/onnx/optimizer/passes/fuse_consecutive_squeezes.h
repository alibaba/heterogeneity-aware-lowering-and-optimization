...
struct FuseConsecutiveSqueezes final : public PredicateBasedPass {
  ...
  bool runTransform(Node* n, Graph& graph,
    ...
    if (axes1_initializer == graph.initializers().end()) return false;
    ...
    if (axes2_initializer == graph.initializers().end()) return false;
    ...
    t.sizes() =
        std::vector<int64_t>{static_cast<int64_t>(combined_axes.size())};
    auto& data = t.int64s();
