...
void propagateElemTypeWithValidation(const TypeProto* input_type,
                                     TypeProto* output_type);

inline void propagateTensorElemTypeWithValidation(const TypeProto* input_type,
                                                  TypeProto* output_type) {
...
inline void propagateSequenceElemTypeWithValidation(const TypeProto* input_type,
                                                    TypeProto* output_type) {
  if (nullptr == input_type) {
    fail_type_inference("Input type was null");
  }
  ...
  if (input_type->value_case() != TypeProto::kSequenceType) {
    fail_type_inference("Input was expected to have sequence type. Got ",
                        input_type->value_case());
  }
...
inline void propagateElemTypeWithValidation(const TypeProto* input_type,
                                            TypeProto* output_type) {
  if (nullptr == input_type) {
    fail_type_inference("Input type was null");
  }
...
inline void propagateElemTypeFromTensorInputToOutput(InferenceContext& ctx,
                                                     size_t inputIndex,
                                                     size_t outputIndex) {
...
inline void propagateElemTypeFromSequenceInputToOutput(InferenceContext& ctx,
                                                       size_t inputIndex,
                                                       size_t outputIndex) {
  auto input_type = ctx.getInputType(inputIndex);
  if (nullptr == input_type ||
      input_type->value_case() != TypeProto::kSequenceType) {
    fail_type_inference("Input ", inputIndex,
                        " expected to have sequence type");
  }
  auto input_seq_type = input_type->sequence_type();
  if (input_seq_type.has_elem_type() &&
      input_seq_type.elem_type().has_tensor_type()) {
    if (input_seq_type.elem_type().tensor_type().elem_type() ==
    ...
    if (output_type->value_case() == TypeProto::kSequenceType ||
      ...
      output_type->mutable_sequence_type()
          ->mutable_elem_type()
          ->mutable_tensor_type()
          ->set_elem_type(input_seq_type.elem_type().tensor_type().elem_type());
    } else {
      fail_type_inference("Output ", outputIndex,
                          " expected to have sequence type");
    }
  }
}

inline void propagateElemTypeFromInputToOutput(InferenceContext& ctx,
                                               size_t inputIndex,
                                               size_t outputIndex) {
  auto input_type = ctx.getInputType(inputIndex);
  if (nullptr == input_type) {
...
inline void UnionTypeInfo(const TypeProto& source_type,
                          TypeProto& target_type) {
  if (source_type.value_case() != target_type.value_case()) {
    fail_type_inference(
        "Mismatched type:", " source=", source_type.value_case(),
        " target=", target_type.value_case());
  }
  ...
  if (target_type.has_tensor_type()) {
    ...
    if (source_elem_type != target_elem_type) {
      fail_type_inference("Mismatched tensor element type:", " source=",
                          source_elem_type, " target=", target_elem_type);
    }

    UnionShapeInfo(source_type.tensor_type().shape(),
    ...
    UnionTypeInfo(source_type.sequence_type().elem_type(),
                  *target_type.mutable_sequence_type()->mutable_elem_type());
  }
}
