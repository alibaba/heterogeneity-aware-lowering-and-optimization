...
struct FunctionBodyBuildContextImpl : public FunctionBodyBuildContext {
  // Input_types: use a default TypeProto for missing types. We use a different
  // convention here (from FunctionBodyBuildContext) to simplify python
  // interoperability.
  FunctionBodyBuildContextImpl(const NodeProto& node_proto,
  ...
  bool hasInput(int inputIndex) const {
    if (inputIndex >= node_proto_.input_size()) return false;
    return node_proto_.input(inputIndex) != "";
  }

  bool hasOutput(int inputIndex) const {
    if (inputIndex >= node_proto_.output_size()) return false;
  ...
  const TypeProto* getInputType(int inputIndex) const {
    ...
    if (j >= input_types_.size()) return nullptr;
...
class OpSchema;
using ContextDependentFunctionBodyBuilder = std::function<bool(
  ...
  class FormalParameter final {
   public:
    ...
    explicit FormalParameter(
        std::string name, DataTypeSet allowed_type_set, std::string type_str,
        const std::string& description,
        FormalParameterOption param_option = Single, bool is_homogeneous = true,
        int min_arity = 1,
    ...
    explicit FormalParameter(
        std::string name, const std::string& description, std::string type_str,
        FormalParameterOption param_option = Single, bool is_homogeneous = true,
        int min_arity = 1,
        DifferentiationCategory differentiation_category = Unknown)
        : name_(std::move(name)),
...
#define ATTR_SETTER_WITH_DEFAULT_VALUE(TypeName) \
  ...
  // Non-STL wrapper to reduce binary size
  OpSchema& Input(int n, const char* name, const char* description,
                  const char* type_str,
                  FormalParameterOption param_option = Single,
                  bool is_homogeneous = true, int min_arity = 1,
                  DifferentiationCategory differentiation_category = Unknown);
  ...
  // Non-STL wrapper to reduce binary size
  OpSchema& Output(int n, const char* name, const char* description,
                   const char* type_str,
                   FormalParameterOption param_option = Single,
                   bool is_homogeneous = true, int min_arity = 1,
                   DifferentiationCategory differentiation_category = Unknown);
  ...
  bool BuildContextDependentFunction(const FunctionBodyBuildContext& ctx,
 ...
 public:
  ...
  class DomainToVersionRange final {
   public:
    ...
    void AddDomainToVersion(const std::string& domain, int min_version,
                            int max_version, int last_release_version = -1) {
      ...
      if (last_release_version == -1) last_release_version = max_version;
      assert(last_release_version_map_.end() ==
...
#define ONNX_TRAINING_OPERATOR_SET_SCHEMA(name, ver, impl)                 \
  ONNX_OPERATOR_SET_SCHEMA_EX(name, OnnxTraining, AI_ONNX_TRAINING_DOMAIN, \
                              ver, true, impl)
