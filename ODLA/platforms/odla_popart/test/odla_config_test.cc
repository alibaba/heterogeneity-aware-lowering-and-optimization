...
TEST_CASE("TestConfig") {
  ...
  SUBCASE("TestCompLoadCache") 
   {
      ...
      //use cache (test.json)
      {
        ...
        CHECK_EQ(PopartConfig::instance()->execution_mode(), ExecutionMode::SEQUENCE);
      }
  ...
  SUBCASE("TestConfigGetFunction")
  {
    ...
    pci->get_pipeline_setting("Mul", ipu_idx,
        pipeline_stage);
    CHECK_EQ(1, ipu_idx);
  ...
  SUBCASE("TestUnexpectedSetting")
  {
    //test odla_SetComputationItem 1001, 1002
    {
      ...
      CHECK_EQ(odla_SetComputationItem(comp, (odla_item_type) 1001, (odla_item_value)cache_dir.c_str()), ODLA_SUCCESS);
      
      //unsuport type 1002 
      CHECK_EQ(odla_SetComputationItem(comp, (odla_item_type) 1002, nullptr), ODLA_UNSUPPORTED_DATATYPE);
  ...
  SUBCASE("TestConfigFunctionCallAfterInited")
   {
      ...
      CHECK_EQ(std::string("ContextQueues"), PopartConfig::instance()->queue_type());
      CHECK_EQ(512 * 512,  PopartConfig::instance()->queue_capacity());
      CHECK_EQ(ExecutionMode::PARALLEL,  PopartConfig::instance()->execution_mode());

      CHECK_NOTHROW_MESSAGE(PopartConfig::instance()->parse_from_json(config_json),  "config already inited");
      CHECK_NOTHROW_MESSAGE(PopartConfig::instance()->load_config(path.c_str()),  "config already inited");
   }
