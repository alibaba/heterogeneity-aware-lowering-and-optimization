...
odla_values odla_CustomOp(odla_values inputs, const odla_char* op_name,
  ...
  if (std::string(op_name) == "custom_IpuGelu") {
    ...
    const popart::OperatorIdentifier attention_mask(
        popart::Domain::ai_graphcore, "AttentionMask", 1, 2, 1);
    auto input_mask = inputs.values[0];
    auto data = inputs.values[1];
    popart::TensorId result = g_comp->builder->customOp(
        attention_mask, 1, {input_mask->tensor_id, data->tensor_id}, 1,
    ...
    ret.size = 1;
    ret.values[0] = val;
    // std::cout << "custom_IpuAttentionMask finished and will return the value"
    // << std::endl;
  } else {
    assert(0);
  }
