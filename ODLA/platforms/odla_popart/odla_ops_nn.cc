...
odla_value odla_AveragePool(
  ...
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "AveragePool";
  ...
  padding.insert(padding.end(), padding_from_back.begin(), padding_from_back.end());
  popart::TensorId result =
      g_comp->builder->aiOnnxOpset10().averagepool({input->tensor_id},
                                                  kernel_shape,
                                                  0,
                                                  0, 
                                                  padding, 
...
odla_value odla_BatchNormalization(
  ...
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "BatchNormalization";
  ...
  if (scale == NULL) {
    ...
    scale = odla_CreateConstant({input_type, {.size = 1, .dims = {channel_dim}}}, scale_tmp.data(), (const odla_value_id)"scale");
  }
  if (offset == NULL) {
    std::vector<float> offset_tmp(channel_dim, scalar_offset);
    offset = odla_CreateConstant({input_type, {.size = 1, .dims = {channel_dim}}}, offset_tmp.data(), (const odla_value_id)"offset");
  }

  auto outs = g_comp->builder->aiOnnxOpset10().batchnormalization(
      {input->tensor_id, scale->tensor_id, offset->tensor_id, mean->tensor_id, var->tensor_id}, 1, epsilon, 0.9);
...
odla_value odla_Conv(odla_value input, odla_memory_layout input_layout, odla_uint32 group,
          ...
          const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "Conv";
  ...
  std::vector<int64_t> kernel_shape(g_comp->builder->getTensorShape(kernel->tensor_id));
  if (kernel_shape.size() > 2) {
    kernel_shape.erase(kernel_shape.begin(), kernel_shape.begin() + 2);
  }

  auto result = g_comp->builder->aiOnnxOpset10().conv(
      {input->tensor_id, kernel->tensor_id}, dim_dilations, group, kernel_shape, pads, dim_strides); 
  return new _odla_value(result,
                         {g_comp->builder->getTensorDataType(result),
                          g_comp->builder->getTensorShape(result)},
                         name);
...
odla_value odla_DeConv(odla_value input, odla_memory_layout input_layout,
            ...
            odla_value_shape output_dims, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "DeConv";
  ...
  auto result = g_comp->builder->aiOnnxOpset10().convtranspose({input->tensor_id, kernel->tensor_id}, 
                                                              dim_dilations, 
                                                              group, 
                                                              kernel_shape, 
...
odla_value odla_Elu(odla_value input, odla_float32 alpha, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "Elu";
...
// odla_values odla_GRU(odla_value input, odla_value_shape weight_dims, odla_value W,
...
//   const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "GRU";
//   std::string direction_name = GetDirectionName(direction);
//   int linear_before_reset = 0; 

//   auto outs = g_comp->builder->aiOnnxOpset10().gru({input->tensor_id, W->tensor_id, R->tensor_id, B->tensor_id}
//                                                    ,2                   // num_outputs
...
//   odla_value value_1 = new _odla_value(outs[0],
//                                        {g_comp->builder->getTensorDataType(outs[0]),
//                                         g_comp->builder->getTensorShape(outs[0])},
//                                        name + "1");
//   odla_value value_2 = new _odla_value(outs[1],
//                                        {g_comp->builder->getTensorDataType(outs[1]),
//                                         g_comp->builder->getTensorShape(outs[1])},
//                                        name + "2");

//   return std::move(odla_values{2, {value_1, value_2}});

// }

odla_value odla_HardSigmoid(odla_value input, odla_float32 alpha, odla_float32 beta,
                 const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "HardSigmoid";

  popart::TensorId result =
      g_comp->builder->aiOnnxOpset10().hardsigmoid({input->tensor_id}, alpha, beta);
  return new _odla_value(result,
                         {g_comp->builder->getTensorDataType(result),
                          g_comp->builder->getTensorShape(result)},
                         name);
...
odla_value odla_InstanceNormalization(
    ...
    odla_float32 scalar_scale, odla_float32 scalar_offset,
    const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "InstanceNormalization";
  ...
  if (scale == NULL) {
    std::vector<float> scale_tmp(channel_dim, scalar_scale);
    scale = odla_CreateConstant({ODLA_FLOAT32, {.size = 1, .dims = {channel_dim}}}, scale_tmp.data(), (const odla_value_id)"scale");
  }
  if (offset == NULL) {
    std::vector<float> offset_tmp(channel_dim, scalar_offset);
    offset = odla_CreateConstant({ODLA_FLOAT32, {.size = 1, .dims = {channel_dim}}}, offset_tmp.data(), (const odla_value_id)"offset");
  }

  auto result = g_comp->builder->aiOnnxOpset10().instancenormalization(
      {input->tensor_id, scale->tensor_id, offset->tensor_id}, epsilon);
      // {input->tensor_id, scale->tensor_id, offset->tensor_id,mean->tensor_id, var->tensor_id}, epsilon);
  return new _odla_value(result,
                         {g_comp->builder->getTensorDataType(result),
                          g_comp->builder->getTensorShape(result)},
                         name);
...
odla_value odla_LeakyRelu(
    odla_value input, odla_float32 alpha, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "LeakyRelu";
...
extern odla_value odla_LogSoftmax(
    odla_value input, odla_int32 axis, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "LogSoftmax";
...
odla_values odla_LSTM(odla_value input, odla_value_shape weight_dims, odla_value W,
          odla_value R, odla_value B, odla_uint32 seq_len,
          odla_int32 hidden_size, odla_rnn_direction direction,
          odla_rnn_outputs outputs, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "LSTM";
  std::string direction_name = GetDirectionName(direction);
  int input_forget = 0; 

  auto outs = g_comp->builder->aiOnnxOpset10().lstm({input->tensor_id, W->tensor_id, R->tensor_id, B->tensor_id},
                                                    3,                   // num_outputs
                                                    std::vector<float> (),  //activation_alpha
                                                    std::vector<float> (),  //activation beta
                                                    std::vector<std::string> (), //activations
  ...
  odla_value value_2 = new _odla_value(outs[1],
                                       {g_comp->builder->getTensorDataType(outs[1]),
                                        g_comp->builder->getTensorShape(outs[1])},
                                       name + "1");
  odla_value value_3 = new _odla_value(outs[2],
...
odla_value odla_MaxPool(
    odla_value input, odla_memory_layout input_layout,
    const odla_uint32* window_dims, const odla_uint32* strides,
    const odla_uint32* paddings_front, const odla_uint32* paddings_back,
    odla_value_shape output_dims, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "MaxPool";
...
odla_value odla_PRelu(odla_value input, odla_float32 slope, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "PRelu";

  std::vector<int64_t> input_shape = g_comp->builder->getTensorShape(input->tensor_id);
  int mul_total = 1;
  ...
  auto sloap = odla_CreateConstant({ODLA_FLOAT32, dim_shape}, sloap_tmp.data(), (const odla_value_id)"sloap");
 
  popart::TensorId result =
      g_comp->builder->aiOnnxOpset10().prelu({input->tensor_id, sloap->tensor_id});
  return new _odla_value(result,
                         {g_comp->builder->getTensorDataType(result),
                          g_comp->builder->getTensorShape(result)},
                         name);
...
// odla_value odla_ROIAlign(odla_value input, odla_memory_layout input_layout,
//               odla_value rois, odla_int32 sampling_ratio, odla_float32 scale,
//               odla_value_shape output_dim, const odla_value_id value_id) {
//   const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "ROIAlign";

//   // output_dims suppose to be 2 * 2
...
//   int nums_rois = 0;
//   if (rois_shape.size() > 1) {
//     nums_rois = rois_shape[0];
//   } 
...
// odla_value ODLA_API_CALL odla_ROIMaxPool(
//     odla_value input, odla_memory_layout input_layout, odla_value rois,
//     odla_int32 win_h, odla_int32 win_w, odla_float32 scale,
//     odla_value_shape output_dim, const odla_value_id value_id) {
//   const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "ROIMaxPool";
//   std::vector<int64_t> pool_shape{win_h, win_w};

//   popart::TensorId result =
//       g_comp->builder->aiOnnxOpset10().maxroipool({input->tensor_id, rois->tensor_id},
//                                                   pool_shape,
//                                                   scale);
//   return new _odla_value(result,
//                          {g_comp->builder->getTensorDataType(result),
...
odla_value odla_Selu(odla_value input, odla_float32 alpha, odla_float32 gamma,
          const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "Selu";
...
odla_value odla_Sigmoid(odla_value input, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "Sigmoid";
...
odla_value odla_Tanh(odla_value input, const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "Tanh";
...
odla_value odla_TopK(odla_value input, odla_uint32 K, odla_bool largest, odla_bool sorted,
          odla_uint32 axis, odla_value_type output_value_type,
          const odla_value_id value_id) {
  const auto& name = value_id ? std::string(reinterpret_cast<const char*>(value_id)) : "Topk";
  int64_t K_value[] = {K};
  auto K_tensor = odla_CreateConstant({ODLA_INT64, {.size=1, .dims={1}}}, K_value, (const odla_value_id)"K");
  
  std::vector<popart::TensorId> results =
      g_comp->builder->aiOnnxOpset10().topk({input->tensor_id, K_tensor->tensor_id}, axis);
