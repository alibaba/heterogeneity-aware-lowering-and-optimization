...
struct _odla_context {
  ...
  _odla_context(odla_computation comp) : comp(comp) {
    ...
    if (comp->is_dynamic_batch) {
        ...
        for (int i = 0; i < comp->inputs_names.size(); i++) {
            ...
            for (int j = 0; j < input_value->type.shape.size; j++) {
              ...
              if(j != input_value->type.shape.size -1)
                  cfg_string += ", ";
            }
            cfg_string += R"(],
              "max": [)";
    ...
    FixedCalibData calib_data(op_dims, magicmind::DataType::FLOAT32, data_paths.size(), data_paths);
    // perform calibration on network
    auto algorithm = magicmind::QuantizationAlgorithm::SYMMETRIC_LINEAR_ALGORITHM;
    std::unique_ptr<magicmind::ICalibrator> calibrator(magicmind::CreateICalibrator({&calib_data}));
    calibrator->SetQuantizationAlgorithm(algorithm);
    calibrator->Calibrate(comp->network);
#endif
    model = comp->builder->BuildModel("magicmind_model", comp->network, builder_cfg);
...
odla_value odla_CreateConstant(odla_value_type type, const void* ptr,
                               const odla_value_id id) {
  auto c = g_comp->network->AddIConstNode(GetMMDataType(type.element_type),
                                          GetMMDims(type.shape),
...
odla_status odla_BindToArgument(odla_value value, const odla_void* data_ptr,
  ...
  if((iter == context->input_ptrs.end()) || (iter->second.len < bytes)) {
    ...
    context->input_ptrs[value->tensor->GetTensorName()] = {.host_ptr = data_ptr,
                                                   .dev_ptr = dev_ptr,
                                                   .len = bytes};
  ...
  CNRT_CHECK(cnrtMemcpy(context->input_ptrs[value->tensor->GetTensorName()].dev_ptr,
...
odla_status odla_BindToOutput(odla_value value, odla_void* data_ptr,
  ...
  size_t bytes =
      GetTotalElements(real_shape) * GetElementSize(value->type.element_type);
  if (g_comp->is_dynamic_batch) {
    context->output_ptrs[value->tensor->GetTensorName()] = {.host_ptr = data_ptr,
                                                   .dev_ptr = nullptr, 
                                                   .len = bytes,
                                                   .vt = value->type};
    return ODLA_SUCCESS;
...
odla_status odla_ExecuteComputation(odla_computation comp, odla_context context,
  ...
  for (int i = 0; i < comp->input_tensors.size(); i++) {
    ...
    auto dev_ptr = context->input_ptrs[comp->input_tensors[i]->GetTensorName()].dev_ptr;
    CHECK(context->irt_input_tensors[i]->SetData(dev_ptr));
  }
  if (!comp->is_dynamic_batch) {
    CHECK(context->ctx->InferOutputShape(context->irt_input_tensors, context->irt_output_tensors));
    for(int i = 0; i < comp->output_tensors.size(); i++) {
      auto dev_ptr = context->output_ptrs[comp->output_tensors[i]->GetTensorName()].dev_ptr;
      CHECK(context->irt_output_tensors[i]->SetData(dev_ptr));
    }
  }
  if (!comp->is_dynamic_batch) {
    CHECK(context->ctx->Enqueue(context->irt_input_tensors, context->irt_output_tensors,
            context->queue));
  } else {
    CHECK(context->ctx->Enqueue(context->irt_input_tensors, &context->irt_output_tensors,
            context->queue));
  ...
  for(int i = 0; i < comp->output_tensors.size(); i++) {
     ...
     if (!comp->is_dynamic_batch) {
       output_dev_ptr = context->output_ptrs[comp->output_tensors[i]->GetTensorName()].dev_ptr;
     } else {
       output_dev_ptr = context->irt_output_tensors[i]->GetMutableData();
     }
     auto output_host_ptr = context->output_ptrs[comp->output_tensors[i]->GetTensorName()].host_ptr;
     LOG_PRINT("copy output mlu %p size %lu\n", output_dev_ptr, context->irt_output_tensors[i]->GetSize());
     CNRT_CHECK(cnrtMemcpy(output_host_ptr, output_dev_ptr, context->irt_output_tensors[i]->GetSize(),
            CNRT_MEM_TRANS_DIR_DEV2HOST));
  }
  
  if (comp->is_dynamic_batch) {
      for(auto tensor : context->irt_output_tensors)
          tensor->Destroy();
...
static odla_value_shape GetBroadcastOutputShape(odla_value_shape dims_lhs,
  ...
  for (int i = 0; i < ret_shape.size; i++) {
      if ((ret_shape.dims[i] > 0) && 
          (reshape_dims.dims[i] > 0) && 
...
static odla_value binary_op(magicmind::IElementwise op_type, odla_value lhs,
  ...
  auto op = g_comp->network->AddIElementwiseNode(lhs_tensor, rhs_tensor, op_type);
...
odla_value odla_BatchNormalization(odla_value input,
  ...
  magicmind::ITensor* scale_data = (scale && scale->const_layer ?
		  scale->const_layer->GetOutput(0) : nullptr);
  magicmind::ITensor* offset_data = (offset && offset->const_layer ?
		  offset->const_layer->GetOutput(0) : nullptr);

  auto bn = g_comp->network->AddIFusedBatchNormNode(input->tensor, mean_data,
                                      var_data, scale_data, offset_data);
...
odla_value odla_Conv(odla_value input, odla_memory_layout input_layout,
  ...
  auto bias_tensor = (bias && bias->tensor ?
	 	  bias->tensor : nullptr);
  auto conv = g_comp->network->AddIConvNode(input->tensor, weights_tensor, bias_tensor);
  CHECK(conv->SetStride(strides[0], strides[1]));
  CHECK(conv->SetPad(paddings_front[0], paddings_back[0],
           paddings_front[1], paddings_back[1]));
...
odla_value odla_Gemm(odla_value lhs, odla_bool transpose_lhs, odla_value rhs,
  ...
  auto matmul = g_comp->network->AddIMatMulNode(lhs->tensor,
  ...
  return CreateValue(matmul, odla_value_type{lhs->type.element_type, output_dims},
                     id);
}
...
odla_value odla_MaxPool(odla_value input, odla_memory_layout input_layout,
  ...
  pooling->SetPad(paddings_front[0], paddings_back[0],
                   paddings_front[1], paddings_back[1]);
...
odla_value add_reduce_node(magicmind::IReduce type, odla_value input,
  ...
  magicmind::IConstNode* reduce_shape = g_comp->network->AddIConstNode(
      ...
      &reduce_dims[0]);
  auto reduce = g_comp->network->AddIReduceNode(input->tensor,
      reduce_shape->GetOutput(0), type, false);
...
odla_value odla_ReduceMean(odla_value input, odla_size_t num_of_axes,
                           ...
                           odla_value_shape output_dims,
                           const odla_value_id id) {
  return add_reduce_node(magicmind::IReduce::MEAN, input, num_of_axes, axes, keep_dims,
		  output_dims, id);
}
...
odla_value odla_ReduceMax(odla_value input, odla_size_t num_of_axes,
                          ...
                          odla_value_shape output_dims,
                          const odla_value_id id) {
  return add_reduce_node(magicmind::IReduce::MAX, input, num_of_axes, axes, keep_dims,
		  output_dims, id);
}
...
odla_value odla_ReduceSum(odla_value input, odla_size_t num_of_axes,
                          ...
                          odla_value_shape output_dims,
                          const odla_value_id id) {
  return add_reduce_node(magicmind::IReduce::ADD, input, num_of_axes, axes, keep_dims,
		  output_dims, id);
}

odla_value odla_Relu(odla_value input, const odla_value_id id) {
  auto relu = g_comp->network->AddIActivationNode(input->tensor, magicmind::IActivation::RELU);
...
odla_value odla_Clamp(odla_value input, odla_float32 lo, odla_float32 hi,
  ...
  auto clip = g_comp->network->AddIClipNode(input->tensor, 
		  lo_node->GetOutput(0), hi_node->GetOutput(0));
...
odla_value odla_Gather(odla_value input, const odla_value indices,
  ...
  auto gather = g_comp->network->AddIGatherNode(input->tensor,
		  indices->tensor, axis_node->GetOutput(0));
...
odla_value odla_Slice(odla_value input, const odla_int32* start,
  ...
  magicmind::IConstNode *begin_node = g_comp->network->AddIConstNode(
		  ...
		  magicmind::Dims({dims.size}), (void *)start);
  ...
  if (is_dynamic_shape) {
      auto shape_node = g_comp->network->AddIShapeNode(
      ...
      if (dims.size > 1) {
          ...
          auto temp_node = g_comp->network->AddIConstNode(
                            magicmind::DataType::INT32,
                            magicmind::Dims({dims.size}), &temp_data[0]);
          end_node = g_comp->network->AddIElementwiseNode(
                            shape_node->GetOutput(0),
                            temp_node->GetOutput(0),
      ...
      end_node = g_comp->network->AddIConstNode(
                   magicmind::DataType::INT32,
                   magicmind::Dims({dims.size}), (void*)end);
  }
  magicmind::IConstNode *stride_node = g_comp->network->AddIConstNode(
		  magicmind::DataType::INT32, 
		  magicmind::Dims({dims.size}), (void *)stride);
  auto slice = g_comp->network->AddIStridedSliceNode(input->tensor, begin_node->GetOutput(0),
                  end_node->GetOutput(0), stride_node->GetOutput(0), nullptr);
...
odla_value odla_Softmax(odla_value input, odla_int32 axis,
  ...
  auto sm = g_comp->network->AddISoftmaxNode(input->tensor, axis_node->GetOutput(0));
...
odla_value odla_Tanh(odla_value input, const odla_value_id id) {
  auto op = g_comp->network->AddIActivationNode(input->tensor, magicmind::IActivation::TANH);
...
odla_value odla_Transpose(odla_value input, odla_value_shape permutations,
  ...
  auto permute = g_comp->network->AddIPermuteNode(
              input->tensor,
...
odla_value odla_ArgMax(odla_value input, odla_int32 axis, odla_bool keep_dims,
  ...
  auto dim_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
		  magicmind::Dims({1}), &reduce_axes);
  auto reduce_max = g_comp->network->AddIReduceNode(input->tensor, dim_node->GetOutput(0),
		 magicmind::IReduce::MAX, true);
...
odla_value odla_Concat(odla_values inputs, odla_int32 axis,
  ...
  auto dim_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
		  magicmind::Dims({1}), &axis);
  auto concat = g_comp->network->AddIConcatNode(dim_node->GetOutput(0),
		  input_tensors);
...
odla_value odla_Not(odla_value input, const odla_value_id id) {
  auto op = g_comp->network->AddILogicNode(input->tensor, magicmind::ILogic::NOT);
  return CreateValue(op, input->type, id);
}
...
odla_value odla_OneHot(odla_value indices, odla_int32 depth, odla_value values,
  ...
  int off_value = 0;
  auto off_value_node = g_comp->network->AddIConstNode(data_type,
             magicmind::Dims({1}), &off_value);
  auto depth_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
             magicmind::Dims({1}), &depth);
  auto op = g_comp->network->AddIOneHotNode(indices->tensor, depth_node->GetOutput(0),
		  values->tensor, off_value_node->GetOutput(0));
...
odla_value odla_Pad(odla_value input, const odla_uint32* padding_front,
  ...
  auto pad_data_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
             magicmind::Dims({input_dims.size, 2}), &padding_data[0]);
...
odla_value odla_Select(odla_value condition, odla_value a, odla_value b,
                       odla_value_shape output_dims,
                       const odla_value_id id) {
  auto op = g_comp->network->AddISelectNode(a->tensor, b->tensor, condition->tensor);
...
odla_value odla_Squeeze(odla_value input, odla_size_t num_of_axes,
  ...
  auto axes_data_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
             magicmind::Dims({(int64_t)num_of_axes}), &axes_to_squeeze[0]);
  auto op = g_comp->network->AddISqueezeNode(input->tensor, axes_data_node->GetOutput(0));
  return CreateValue(op, {input->type.element_type, output_dims}, id);
}
...
odla_value odla_Tile(odla_value input, const odla_uint32* repeat,
  ...
  auto repeat_data_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
             magicmind::Dims({(int64_t)dims}), &repeat_data[0]);
...
odla_values odla_TopK(odla_value input, odla_uint32 K, odla_bool largest,
  ...
  auto k_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
             magicmind::Dims({1}), &K);
  auto topk = g_comp->network->AddITopKNode(input->tensor, k_node->GetOutput(0));
...
odla_status odla_EnterBranchBody(odla_bool true_branch) {
  ...
  if (g_comp->branchs.top().in_true_body) {
    g_comp->network = static_cast<magicmind::INetwork*>
        (g_comp->branchs.top().then_body);
  } else {
    g_comp->network = static_cast<magicmind::INetwork*>
        (g_comp->branchs.top().else_body);
  }
}
...
odla_values odla_EndIf(odla_value_ids value_ids) {
  ...
  for (int i = 0; i < n; ++i) {
    ret.values[i] = CreateValue(br->GetOutput(i),
                                br_info.true_outputs[i]->type,
...
odla_value odla_SliceDynamic(odla_value input, odla_value start,
                             ...
                             odla_value_shape output_dims, 
                             const odla_value_id id) {
  auto slice = g_comp->network->AddISliceNode(input->tensor,
                                              start->tensor, size->tensor);
  return CreateValue(slice, {input->type.element_type, output_dims}, id);
}

odla_value odla_Stack(odla_values inputs, odla_int32 axis, odla_value_shape output_shape,
                      const odla_value_id id) {
  int num = inputs.size;
  std::vector<magicmind::ITensor*> input_tensors(num);
  for (int i = 0; i < num; ++i) {
  ...
  auto dim_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
        magicmind::Dims({1}), &axis);
  auto pack = g_comp->network->AddIPackNode(dim_node->GetOutput(0),
                                             input_tensors);
  return CreateValue(pack, {inputs.values[0]->type.element_type, output_shape}, id);
}
odla_value odla_ReduceL2(odla_value input, odla_size_t num_of_axes, 
    ...
    if (num_of_axes == 1) {
      auto dim_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
                                             magicmind::Dims({1}), (int*)axes);
      auto normalize = g_comp->network->AddINormalizeNode(input->tensor,
                                dim_node->GetOutput(0), nullptr);
      normalize->SetEpsilon(epsilon);
      return CreateValue(normalize, {input->type.element_type, output_dims}, value_id);
    } else {
        return input;
    }
}
...
odla_values odla_CustomOp(odla_values inputs, const odla_char* op_name,
  ...
  if (std::string(op_name) == "DetectionOutput") {
    ...
    // permute loc
    auto loc_permute =
            g_comp->network->AddIPermuteNode(loc_reshape_node->GetOutput(0),
                                             permute_perms->GetOutput(0));
    ...
    // permute conf
    auto conf_permute =
            g_comp->network->AddIPermuteNode(conf_reshape_node->GetOutput(0),
                                             permute_perms->GetOutput(0));
    ...
    auto detection_output = g_comp->network->AddIDetectionOutputNode(det_input, prior_permute->GetOutput(0));
    detection_output->SetAlgo(magicmind::IDetectionOutputAlgo::SSD);
    detection_output->SetBackgroundLabelId((int64_t)bg_label_id);
    detection_output->SetCodeType(GetMMDetectionCodeType(code_type));//CENTER_SIZE
...
odla_values odla_Split(odla_value input, odla_value split_dim, odla_int32 num_split,
           const odla_value_ids value_ids) {
  auto split_node = g_comp->network->AddISplitNode(input->tensor, split_dim->tensor, 
             (int64_t(num_split)));
  odla_values ret;
  ret.size = num_split;
  for (int i = 0; i < ret.size; i++) {
    ret.values[i] = CreateValue(split_node->GetOutput(i), 
                                input->type, value_ids.value_ids[i]);
  }
  return ret;
}
odla_value odla_LpNormalize(odla_value input, odla_int32 p, 
                            ...
                            const odla_value_id value_id) {
  auto dim_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
                               magicmind::Dims({(int64_t)axes_size}), (int*)axes);
  ...
  if (scale_tensor) {
    if (scale->type.shape.size == 1) {
      ...
      auto shape_data_node = g_comp->network->AddIConstNode(magicmind::DataType::INT32,
                               magicmind::Dims({1}), &scale_total_size);
      auto reshape_node = g_comp->network->AddIReshapeNode(scale_tensor,
                               shape_data_node->GetOutput(0));
      scale_tensor = reshape_node->GetOutput(0);
    }
  }
  auto normalize = g_comp->network->AddINormalizeNode(input->tensor,
                                dim_node->GetOutput(0), scale_tensor);
  normalize->SetP((float)p);
  normalize->SetEpsilon(epsilon);
  if (scale_tensor) {
    //caffe case
    if (axes_size == 1) {
      normalize->SetAcrossSpatial(magicmind::INormalizeAcrossSpatialMode::NOT_ACROSS_SPATIAL);
    } else {
      normalize->SetAcrossSpatial(magicmind::INormalizeAcrossSpatialMode::ACROSS_SPATIAL);
    }

    if(scale_total_size == 1) {
      normalize->SetChannelShared(magicmind::INormalizeChannelSharedMode::CHANNEL_SHARED);
    } else {
      normalize->SetChannelShared(magicmind::INormalizeChannelSharedMode::NOT_CHANNEL_SHARED);
    }
  } else {
    normalize->SetAcrossSpatial(magicmind::INormalizeAcrossSpatialMode::ACROSS_AXIS_TENSOR);
  }
