...
void topk_gpu(const dnnl::memory & src, const dnnl::memory & dst, const dnnl::memory & dst_idx_mem,
          SizeVector in_dims, int32_t axis, int before_num, int dim, int src_k, int count_vec, bool sort_value) {
   ...
   // copy(policy, counting_begin, counting_begin + n, vals_begin);
    ...
    transform(policy, counting_begin, counting_begin + n, keys.begin(),
            [n](int i) { return i; });
  // 2. Sorting
  auto zipped_begin = oneapi::dpl::make_zip_iterator(vals.begin(), keys.begin());
  // stable sort by keys
   stable_sort(policy, zipped_begin, zipped_begin + n,[](auto lhs, auto rhs) { return get<0>(lhs) > get<0>(rhs); } );

  std::copy(policy,std::begin(vals),std::begin(vals)+src_k,dst_data);
  std::copy(policy,std::begin(keys),std::begin(keys)+src_k,dst_idx);

  sycl_queue.wait();
  std::copy(dst_data,dst_data+src_k,std:: ostream_iterator<float> {std::cout," "}) ;
  std::cout << std::endl;
  std::copy(dst_idx, dst_idx +src_k,std:: ostream_iterator<int> {std::cout," "}) ;
  std::cout << std::endl;
...
void topk_func_gpu(const dnnl::memory & src, const dnnl::memory& dst, const dnnl::memory & dst_idx,
                      std::vector<int32_t> src_dims, uint32_t K, bool largest,
                      bool sorted, uint32_t axis) {

  std::cout <<"topk_func_gpu " << " K" << K <<"general code " << std::endl;
  ...
  topk_gpu(src, dst, dst_idx, in_dims, axis, before_num,dim, src_k, count_vec, sort_value);
