...
#define CALC_ALIGN_LEN(size) ((size + VODH_ALIGN_SIZE - 1)/VODH_ALIGN_SIZE * VODH_ALIGN_SIZE)
...
vodh_ret vodh_get_total_cap(void *vodh_handle,
	struct vodh_total_cap *cap);
...
vodh_ret vodh_get_one_dev_cap(void *vodh_handle,
	struct vodh_dev* dev, struct vodh_dev_cap *cap);
...
vodh_ret vodh_create_context(void *vodh_handle,
	struct vodh_dev* dev, vodla_context *context);

/* support multithreading */
vodh_ret vodh_destroy_context(void *vodh_handle,
	struct vodh_dev* dev, vodla_context context);

/* support multithreading */
vodh_ret vodh_infer(void *vodh_handle, struct vodh_dev *dev,
	struct vodh_infer_options *options, struct vodh_infer_result *result);

vodh_ret vodh_model(void *vodh_handle,
	struct vodh_dev* dev, struct vodh_model_options *moptions,
