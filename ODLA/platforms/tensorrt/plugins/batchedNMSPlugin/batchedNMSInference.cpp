...
pluginStatus_t nmsInference(
    ...
    void* nmsedClasses, void* nmsedIndices, void* workspace, bool isNormalized,
    bool confSigmoid, bool clipBoxes) {
  ...
  status = gatherNMSOutputs(
      ...
      nmsedBoxes, nmsedScores, nmsedClasses, nmsedIndices, clipBoxes);
  ASSERT_FAILURE(status == STATUS_SUCCESS);
